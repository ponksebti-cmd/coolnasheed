...
export function CommandPalette() {
  ...
  const items = useMemo<Item[]>(() => {
    ...
    const actions: Item[] = [
      { id: "a-play", group: "Actions", label: player.playing ? "Pause" : "Play", icon: player.playing ? "pause" : "play", run: () => { player.toggle(); close(); } },
    ...
    const pages: Item[] = [
      ...
      { id: "p-search", group: "Go to", label: "Search", icon: "search", run: go("/search") },
      { id: "p-library", group: "Go to", label: "Library", icon: "library", run: go("/library") },
      { id: "p-queue", group: "Go to", label: "Queue", icon: "queue", run: go("/queue") },
      { id: "p-about", group: "Go to", label: "About CoolNasheed", icon: "info", run: go("/about") },
      ...library.playlists.map((pl) => ({ id: `p-${pl.id}`, group: "Go to", label: pl.name, sub: `${pl.trackIds.length} tracks`, icon: "rows" as IconName, run: go(`/p/${pl.id}`) })),
    ];

    if (!q.trim()) return [...actions, ...pages];

    const tracks = searchTracks(q).slice(0, 6).map((t) => ({
      id: `t-${t.id}`,
      group: "Nasheeds",
      label: t.title,
    ...
    const sets = searchCollections(q).slice(0, 3).map((c) => ({
      id: `c-${c.id}`,
      group: "Sets",
      label: c.title,
      sub: c.curator,
    ...
    const artists = searchArtists(q).slice(0, 3).map((a) => ({
      id: `ar-${a.id}`,
      group: "Reciters",
      label: a.name,
      sub: a.origin,
  ...
  }, [q, player, library, navigate, setCommand, setNur]);
    ...
    <div className="fixed inset-0 z-[110] flex items-start justify-center p-4 pt-[12vh] veil-enter" role="dialog" aria-modal="true" aria-label="Command palette">
      ...
      <div className="glass materialize relative z-10 w-full max-w-[560px] overflow-hidden rounded-2xl shadow-[0_40px_120px_-30px_rgba(0,0,0,1)]">
        <div className="flex items-center gap-3 border-b border-line px-4 py-3">
          ...
          />
          <span className="hidden shrink-0 rounded border border-line2 px-1.5 py-0.5 text-[10px] font-semibold text-muted sm:block">ESC</span>
        </div>

        <div ref={listRef} className="scroll-slim max-h-[52vh] overflow-y-auto p-1.5">
          {items.length === 0 ? (
            <div className="px-4 py-10 text-center text-sm text-muted">
              Nothing matches “{q}”. Try a maqām (Ḥijāz, Rāst), a city, or a word from a lyric.
            </div>
          ) : (
            groups.map((group) => (
                  ...
                  return (
                    <button
                      ...
                      className={clsx(
                        ...
                        idx === cursor ? "bg-jade/12 text-text" : "text-text2 hover:bg-surface2",
                      )}
                    >
                      <span className={clsx("grid h-7 w-7 shrink-0 place-items-center rounded-md border border-line bg-surface2", idx === cursor && "text-jade")}>
                        <Icon name={item.icon} size={14} />
                      </span>
                      <span className="min-w-0 flex-1">
                        <span className="block truncate text-[13px] font-medium">{item.label}</span>
                        {item.sub ? <span className="block truncate text-[11px] text-muted">{item.sub}</span> : null}
                      </span>
                      {idx === cursor ? <Icon name="arrowUpRight" size={13} className="shrink-0 text-muted" /> : null}
                    </button>
                  );
                })}
              </div>
        ...
        <div className="flex items-center justify-between gap-3 border-t border-line px-4 py-2 text-[10.5px] text-muted">
          ...
          <span className="truncate">{ARTISTS.length} reciters · {COLLECTIONS.length} sets indexed</span>
        </div>
      </div>
    </div>,
    document.body,
