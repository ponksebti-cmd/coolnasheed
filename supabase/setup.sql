...
--   8. migrations/20260917090000_catalogue_window.sql
--
-- Two ways to run it:
--
...
/* ================================================================ 20260917090000_catalogue_window.sql */

do $cn_migration$
begin
  /* Already applied, or already done by something that did not write the ledger. */
  if not exists (select 1 from public.applied_migrations where name = '20260917090000_catalogue_window.sql')
     and (case when to_regprocedure('public.catalog_payload()') is null then false else (select pg_get_functiondef('public.catalog_payload()'::regprocedure) like '%limit 300%') end) then
    insert into public.applied_migrations (name) values ('20260917090000_catalogue_window.sql')
      on conflict (name) do nothing;
  end if;

  if exists (select 1 from public.applied_migrations where name = '20260917090000_catalogue_window.sql') then
    raise notice 'CoolNasheed: 20260917090000_catalogue_window.sql is already applied — skipping';
    return;
  end if;
